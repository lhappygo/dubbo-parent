 ...
 * 协议接口
 *
 * Protocol 是服务域，它是 Invoker（实体域，执行体,可能是local/remote/cluster的实现） 暴露和引用的主功能入口，
 * 它负责 Invoker 的生命周期管理。
 */
@SPI("dubbo")
public interface Protocol {
     ...
     */
    /**
     * 暴露远程服务：<br>  密等性：同一操作发起的一次请求或者多次请求的结果是一致的
     * 1. 协议在接收请求时，应记录请求来源方地址信息：RpcContext.getContext().setRemoteAddress();<br>
     * 2. export() 必须是幂等的，也就是暴露同一个 URL 的 Invoker 两次，和暴露一次没有区别。<br>
     * 3. export() 传入的 Invoker 由框架实现并传入，协议不需要关心。<br>
     *
     * @param <T>     服务的类型
     * @param invoker 服务的执行体（local or remote or cluster impl)
     * @return exporter 暴露服务的引用，用于取消暴露
     * @throws RpcException 当暴露服务出错时抛出，比如端口已占用
     */
    @Adaptive
    <T> Exporter<T> export(Invoker<T> n) throws RpcException;
     ...
     * @param <T>  服务的类型
     * @param type 服务的类型
     * @param url  远程服务的URL地址(remote service URL address)
